...
namespace Inlämninguppgift1.Extensions
{
    ...
    {
        public static void BubbleSort(this List<int> numbers)
        {
        ...
        static void BubbleAnimate(List<int> numbers, int sort)
        {
        ...
        public static void MergeSort(this List<int> numbers)
        {
            if (numbers.Count < 2)
                return;
            ...
            while (step < numbers.Count)
            {
                startL = 0;
                startR = step;
                while (startR + step <= numbers.Count)
                {
                    numbers.Merge(startL, startL + step, startR, startR + step);
                    startL = startR + step;
                    startR = startL + step;
                }
                if (startR < numbers.Count)
                {
                    numbers.Merge(startL, startL + step, startR, numbers.Count);
                }
                step *= 2;
            }
        }
        ...
        private static void Merge(this List<int> numbers, int startL, int stopL,
        ...
        public static void QuickSort(this List<int> numbers)
        {
            numbers.QuickSort(0, numbers.Count - 1);
        }
        static void QuickSort(this List<int> numbers, int low, int high)
        {
            int i = low, j = high;
        ...
        static void SwapElement(this List<int> numbers, int left, int right)
        {
